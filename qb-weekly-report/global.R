...
fetch_qb_stats <- function(season = CURRENT_SEASON, week = NULL) {
  ...
  # Use precomputed per-game stats when available (generated by data-prep.R).
  # This avoids loading the full play-by-play data, which exceeds the 1GB
  # memory limit on shinyapps.io. Regenerate the files after new games are
  # played or when the stat logic below changes.
  cache_file <- file.path("data", paste0("qb_stats_", season, ".rds"))
  if (is.null(week) && file.exists(cache_file)) {
